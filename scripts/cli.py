...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Remove ModSum subparser; keeping MNIST only
    sp.add_argument("--log", type=str, default=None, help="Log file path (default auto under sweep-out/cli_logs).")

    # MNIST weights->metadata training
    fpm = sub.add_parser("mnist-foundation-train", help="Train MNIST weights->metadata Transformer")
    fpm.add_argument("--runs-base", type=str, default="runs/mnist")
    fpm.add_argument("--outdir", type=str, default="runs/mnist_weights2meta")
    fpm.add_argument("--epochs", type=int, default=10)
    fpm.add_argument("--batch-size", type=int, default=32)
    fpm.add_argument("--lr", type=float, default=1e-3)
    fpm.add_argument("--token-size", type=int, default=64)
    fpm.add_argument("--d-model", type=int, default=64)
    fpm.add_argument("--nhead", type=int, default=4)
    fpm.add_argument("--nlayers", type=int, default=2)
    fpm.add_argument("--d-ff", type=int, default=128)
    fpm.add_argument("--dropout", type=float, default=0.1)
    fpm.add_argument("--train-frac", type=float, default=0.9)
    fpm.add_argument("--seed", type=int, default=1337)
    fpm.add_argument("--log", type=str, default=None)
...
def interactive_menu() -> int:
    while True:
        print("\n=== MNIST CLI ===")
        print("1) Run MNIST sweep")
        print("2) Train MNIST (MLP)")
        print("3) Train MNIST Foundation (weights->metadata)")
        print("4) List background jobs")
        print("5) Kill background jobs")
        print("6) Exit")
        ...
        if choice == "1":
            ...
            return interactive_mnist_foundation_train()
        elif choice == "4":
            return list_jobs()
        elif choice == "5":
            return kill_jobs()
...
def interactive_mnist_foundation_train() -> int:
    print("\n[MNIST Foundation Training]")
    runs_base = prompt_str("Runs base dir", "runs/mnist")
    outdir = prompt_str("Output dir", "runs/mnist_weights2meta")
    epochs = prompt_int("Epochs", 10)
    batch_size = prompt_int("Batch size", 32)
    lr = prompt_float("Learning rate", 1e-3)
    ...
    cmd = build_mnist_foundation_cmd(params)
    log = REPO_ROOT / outdir / "cli_logs" / f"mnist_foundation_train_{ts()}.out"
    return run_bg(cmd, log, job_type="mnist-foundation-train", params=params) if bg else run_fg(cmd)
...
def build_mnist_foundation_cmd(params: Dict[str, str]) -> List[str]:
    script = REPO_ROOT / "experiments" / "weights2meta_mnist_train.py"
    cmd = [
        sys.executable,
        "-u",
        str(script),
...
def main(argv: List[str] | None = None) -> int:
    ...
    if argv:
        ...
        if args.cmd == "mnist-foundation-train":
            params = {
            ...
            cmd = build_mnist_foundation_cmd(params)
            log = Path(args.log) if args.log else (REPO_ROOT / args.outdir / "cli_logs" / f"mnist_foundation_train_{ts()}.out")
            return run_fg(cmd) if foreground else run_bg(cmd, log, job_type="mnist-foundation-train", params=params)
