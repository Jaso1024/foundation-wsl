#!/usr/bin/env python3
"""
Train a small Transformer to map saved MNIST MLP weights -> run metadata/metrics.

Input: state_dict from runs/mnist/*/model.pt and matching config.json
...
def list_run_dirs(base: Path) -> List[Path]:
    ...
    if not base.exists():
        return []
    return sorted([p for p in base.glob("*_seed*") if (p / "model.pt").exists() and (p / "config.json").exists()])
...
def load_example(run_dir: Path) -> Tuple[np.ndarray, Dict[str, float]]:
    ...
    y: Dict[str, float] = {
        ...
        "hidden1": float(cfg["hidden1"]),
        "hidden2": float(cfg["hidden2"]),
...
class WeightsTransformer(nn.Module):
    def __init__(self, token_size: int, d_model: int, nhead: int, nlayers: int, dim_feedforward: int, dropout: float, out_dim: int):
        ...
        self.head = nn.Sequential(nn.LayerNorm(d_model), nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, out_dim))
...
def parse_args() -> TrainArgs:
    ap = argparse.ArgumentParser(description="Train transformer to predict MNIST run metadata from weights.")
    ap.add_argument("--runs-base", type=str, default="runs/mnist")
    ap.add_argument("--outdir", type=str, default="runs/mnist_weights2meta")
    ap.add_argument("--epochs", type=int, default=10)
    ap.add_argument("--batch-size", type=int, default=32)
    ap.add_argument("--lr", type=float, default=1e-3)
    ap.add_argument("--token-size", type=int, default=64)
...
def main(argv: List[str] | None = None) -> int:
    ...
    if len(run_dirs) < 10:
        print(f"Not enough runs found under {base} (found {len(run_dirs)}). Generate more with sweep_mnist.py.")
        return 2
